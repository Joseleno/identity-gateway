...
> **Versão 2.4** · Status: aprovada para implementação

---

## 0. O que mudou da v2.3 para a v2.4
...
## 0.1. O que mudou da v2.2 para a v2.3
...
## 0.2. O que mudou da v2.1 para a v2.2
...
## 0.3. O que mudou da v2.0 para a v2.1
...
Se os retries se esgotarem, o tenant vai para `ProvisioningFailed` e o evento fica disponível para retry manual. Um erro **permanente** do adaptador (`IdentityProviderInconsistencyException`, §11.6) não é repetido: vai direto a `ProvisioningFailed`.
    ...
    IUnitOfWork unitOfWork) : ICommandHandler<ProvisionTenantCommand>
{
    public async Task<Result> Handle(ProvisionTenantCommand command, CancellationToken ct)
    {
        ...
        var organizationId = await identity.EnsureOrganizationAsync(tenant.Id, tenant.Slug, tenant.Name, ct);
...
// um consumidor de Fault marca o tenant como ProvisioningFailed. O retry IGNORA
// IdentityProviderInconsistencyException: é erro permanente (§11.6), e repeti-lo
// só adiaria o ProvisioningFailed por configuração, não por decisão.
...
| **A · Fundação Keycloak**: Keycloak no compose e na CI, lado administrativo do realm, service account com `private_key_jwt`, `EnsureOrganizationAsync` | M0 + M1 | Em andamento |
| **B · Consumidor**: transporte, provisionamento, `ProvisioningFailed` | M1 | Próxima |
| **C · Convite do admin inicial** | M1 | Depois da B |
